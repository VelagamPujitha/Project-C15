...
def add_bg_from_url():
    st.markdown(
         ...
         <style>
         .stApp {{
             background-image: url("https://debuglies.com/wp-content/uploads/2020/05/COPD-in-COVID-19-patients.jpg");
...
new_title = '<p style="font-family:sans-serif;  font-size: 42px; color:white">RESPIRATORY DISEASE DETECTION BASED ON LUNG SOUNDS USING DEEP LEARNING</p>'
...
if uploaded_file is not None:
    ...
    if Genrate_pred:
        ...
        st.markdown("<span style='font-size:30px;color:white'>The given Lung Sound is of type : {}</span>".format(type),unsafe_allow_html=True)
        st.markdown("<span style='font-size:30px;color:white'>The chances of Lung Sound being {} is: {}</span>".format(type,accu), unsafe_allow_html=True)